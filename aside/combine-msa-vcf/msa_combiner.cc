...
namespace vcf2multialign {
		...
		switch (seg.type)
		{
			...
			case segment_type::MISMATCH:
			{
				...
				for (auto const &oc : overlap_range)
				{
					...
					auto const &desc(m_variant_deduplicator.handle_variant_description(
						variant_description(
	...
	)
	{
		...
		// Add the variants from the MSA to m_variant_deduplicator.
	...
	)
	{
		...
		// Handle the variant.
		{
			...
			for (auto const &var_alt : alts)
			{
				...
				auto &desc(m_variant_deduplicator.handle_variant_description(variant_description(m_ploidy, gt_count, variant_origin::VC)));
	...
	void msa_combiner::process_variants(msa_segmentation &segmentation)
	{
		...
		auto const initial_variant_count(m_variant_deduplicator.size());
		if (segmentation.overlapping_variants.empty())
		{
			...
			m_variant_deduplicator.merge_output_variants(initial_variant_count);
			m_variant_deduplicator.filter_processed_variants_and_output(SIZE_MAX);
		}
		else
		{
			...
			auto const new_variant_count(m_variant_deduplicator.size());
			auto const handled_count(
				process_variants_msa(
			...
			// Maintain the sorted order.
			m_variant_deduplicator.merge_output_variants(new_variant_count);
			// Output.
			m_variant_deduplicator.filter_processed_variants_and_output(min_unhandled_ref_pos);
		}
